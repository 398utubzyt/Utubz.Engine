...
{
    internal class R2DContext : Object
    {
        public float[] data;
        public uint[] indices;
        ...
        {
            posattr = GL.VertexAttribute.Find(shader, Renderer.DefaultVertexPositionAttribute);
            posattr.Set(dbuf, 3, 3, 0);

            colattr = GL.VertexAttribute.Find(shader, Renderer.DefaultVertexColorAttribute);
            colattr.Set(dbuf, 4, 4, 12);

            texattr = GL.VertexAttribute.Find(shader, Renderer.DefaultVertexTexCoordAttribute);
            texattr.Set(dbuf, 2, 2, 28);

            mvpunif = GL.ShaderUniform.Find(shader, Renderer.DefaultMvpUniform);
